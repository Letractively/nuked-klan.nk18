function init_rating(id_div,theme,NUMBER_OF_STARS){
    ...
    for (var i = 0; i < ratings.length; i++){
        if (ratings[i].className != 'globalRating_'+id_div)
            continue;
        ...
        for (var j = 0; j < NUMBER_OF_STARS; j++){
            ...
            if (rating >= 1){
                ...
                star.setAttribute('src', 'images/rating/stars/'+theme+'/rating_on.png');
                star.className = 'on';
                rating--;
            }else if(rating == 0.5){
                star.setAttribute('style', 'vertical-align:middle');
                star.setAttribute('src', 'images/rating/stars/'+theme+'/rating_half.png');
                star.className = 'half';
                rating = 0;
            }else{
                star.setAttribute('title', j+1);
                star.setAttribute('style', 'vertical-align:middle');
                star.setAttribute('src', 'images/rating/stars/'+theme+'/rating_off.png');
                star.className = 'off';
            }
...
function displayHover(ratingId, star, theme){
    for (var i = 0; i <= star; i++){
        document.getElementById('star_'+ratingId+'_'+i).setAttribute('src', 'images/rating/stars/'+theme+'/rating_over.png');
    }
}
...
function displayNormal(ratingId, star, theme){
    for (var i = 0; i <= star; i++){
        ...
        document.getElementById('star_'+ratingId+'_'+i).setAttribute('src', 'images/rating/stars/'+theme+'/rating_'+status+'.png');
    }
}
